'''
=============================== ABOUT CODING ==================================
Date: Feb 13, 2024. Copy of p2 9v searching for best model.py________________________________________
Task: In p2 9v searching for best model.py, I forgot to save decision with pickle
Now I am saving decision.
...
code_no = 'p24' # just checking a simple 2D CNN based model accuracy
output_path = 'Code Outputs/p24 dataset variation2, mal_3 malignant'
malignancy_3 = 'malignant' #'ignored', 'benign', 'malignant'
...
model_names = ['EfficientNetV2B0'] # ,'ResNet50', 'VGG16', 'VGG19' 'ResNet50', 'EfficientNetV2B0', 'InceptionV3', 'MobileNet', 'NASNetMobile', 'DenseNet121', 'ConvNeXtTiny'
n_freeze = 50 
...
for filename in os.listdir(image_folder):    
    ...
    if malignancy_3 == 'ignored':
        ...
        if nid not in nodule_to_images:
            ...
            # update needed_for stratified sampling only______________update
            if malignancy_3 == 'benign':
                label = 0 if 'c_N' in filename else 1
            elif malignancy_3 == 'malignant':
                label = 1 if 'c_Y' in filename else 0 
            nodule_to_labels[nid] = label
...
# nodule_labels = list(nodule_to_labels.values())
nodule_labels = [nodule_to_labels[key] for key in sorted(nodule_to_images.keys())]
...
# update needed_for stratified sampling only______________update
skf = StratifiedKFold(n_splits=n_fold, shuffle=True, random_state=63)
folds = list(skf.split(nodule_ids, nodule_labels))  
...
# #%%
        ...
        # # printing results to see how stratified sampling works____update
        # val_benign = sum(y_test[test_idx] == 0)
        # val_malignant = sum(y_test[test_idx] == 1)

        # print(f"Fold {fold}:")
        # # print(f"  Train - Benign: {train_benign}, Malignant: {train_malignant}")
        # print(f"  Test - Benign: {val_benign}, Malignant: {val_malignant}")

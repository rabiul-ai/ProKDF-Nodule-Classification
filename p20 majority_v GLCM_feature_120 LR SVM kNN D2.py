'''
=============================== ABOUT CODING ==================================
Date: Feb 18, 2025. Copy of p8 ________________________________________
Task: Checking Performance of GLCM Features, 9views, Number of Features = 50 
Because, n_distance * n_angles * n_features = 2*4*5 = 40
...
code_no = 'p20' # just checking a simple 2D CNN based model accuracy
output_path = 'Code Outputs/p20 majority_v GLCM_feature_120 LR SVM kNN D2'
...
# Define the models
model_names = {
    'LogisticRegression': LogisticRegression(max_iter=20000, solver='saga'),
...
"""====================== Diameter Prior ==================================="""
# SPIE diameter prior KDE
benign_nodule_sizes = np.load(r"C:\Rabiul\1. PhD Research\8. Fall 2024\Lung Project\Coding\5. Coding for Publication\Code Outputs\p32 histogram SPIE dataset dia\all_SPIE_benign_nodule_dia.npy")
malignant_nodule_sizes = np.load(r"C:\Rabiul\1. PhD Research\8. Fall 2024\Lung Project\Coding\5. Coding for Publication\Code Outputs\p32 histogram SPIE dataset dia\all_SPIE_malignant_nodule_dia.npy")
...
'''________________________ Loading GLCM Features____________________________'''
GLCM_feature_filepath = r"C:\Rabiul\1. PhD Research\8. Fall 2024\Lung Project\Coding\5. Coding for Publication\Code Outputs\p7 extracting textures features 9v\SPIE Dataset Features\GLCM_features.xlsx"

GLCM_features = pd.read_excel(GLCM_feature_filepath)
X_value = GLCM_features.iloc[:, 1:-3].values
y_d1 = GLCM_features['d1_label'].values 
y_d2 = GLCM_features['d2_label'].values 
y_d3 = GLCM_features['d3_label'].values


# saving a smaller version_______________________________________
# GLCM_features_1000 = GLCM_features.head(1000)
# GLCM_features_1000.to_excel("GLCM_features_1000.xlsx", index=False)
...
def load_selected_GLCM_features_and_labels(image_list, GLCM_features):
    ...
    GLCM_features_sampled = GLCM_features[GLCM_features['nodule_id'].isin(image_list)]
    GLCM_features_sampled['nodule_id'] = pd.Categorical(GLCM_features_sampled['nodule_id'], categories=image_list, ordered=True)
    GLCM_features_sorted =  GLCM_features_sampled.sort_values('nodule_id')
    X = GLCM_features_sorted.iloc[:, 1:-3].values
    y = GLCM_features_sorted['d1_label'].values 
...
# for model_name in model_names:
    ...
    GLCM_Image_score, GLCM_Image_score_with_dia = {}, {}
    keys = ['y_true_fold','y_pred_fold', 'y_proba_fold', 'y_true_all','y_pred_all', 'y_proba_all']
    for key in keys:
        GLCM_Image_score[key] = []
        GLCM_Image_score_with_dia[key] = []
    ...
    for fold, (train_idx, test_idx) in enumerate(folds):
        ...
        X_train, y_train = load_selected_GLCM_features_and_labels(train_images, GLCM_features)
        X_test, y_test = load_selected_GLCM_features_and_labels(test_images, GLCM_features)
        ...
        GLCM_Image_score['y_true_fold'].append(list(y_test))
        GLCM_Image_score['y_pred_fold'].append(list(y_pred))
        GLCM_Image_score['y_proba_fold'].append(list(y_pred_prob))

        GLCM_Image_score['y_true_all'].extend(list(y_test))
        GLCM_Image_score['y_pred_all'].extend(list(y_pred))
        GLCM_Image_score['y_proba_all'].extend(list(y_pred_prob))
        
        
        '''_______________________ Saving Decision for Fusion with Dia Adjustment__________'''
        GLCM_Image_score_with_dia['y_true_fold'].append(list(y_test))
        GLCM_Image_score_with_dia['y_pred_fold'].append(list(y_pred_adjusted))
        GLCM_Image_score_with_dia['y_proba_fold'].append(list(y_pred_prob_adjusted))

        GLCM_Image_score_with_dia['y_true_all'].extend(list(y_test))
        GLCM_Image_score_with_dia['y_pred_all'].extend(list(y_pred_adjusted))
        GLCM_Image_score_with_dia['y_proba_all'].extend(list(y_pred_prob_adjusted))
    ...
    files_to_be_saved = [GLCM_Image_score, GLCM_Image_score_with_dia]
    file_names = ['GLCM_feature_score', 'GLCM_feature_score_with_dia']
    tag = model_name

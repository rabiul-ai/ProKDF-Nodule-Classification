'''
=============================== ABOUT CODING ==================================
Date: Feb 5, 2024. Copy of p2 ________________________________________
Task: Checking Performance of LBP Images, 9views. ResNet & EfficeintNet
...
code_no = 'p6' # just checking a simple 2D CNN based model accuracy
output_path = 'Code Outputs/p6 LBP Res Eff'
...
model_names = ['ResNet50', 'EfficientNetV2B0'] # ,'VGG16', 'VGG19' 'ResNet50', 'EfficientNetV2B0', 'InceptionV3', 'MobileNet', 'NASNetMobile', 'DenseNet121', 'ConvNeXtTiny'
n_freeze = 50 
...
image_folder = r"C:\Rabiul\1. PhD Research\8. Fall 2024\Lung Project\Coding\1. Restart Coding\Code Outputs\rc17 save 3D nodule 32\2D Nodules 64\LBP Images"
...
for model_name in model_names:
    ...
    for fold, (train_idx, test_idx) in enumerate(folds):
        ...
        LBP_Image_score['y_true_fold'].append(y_test.tolist())
        LBP_Image_score['y_pred_fold'].append(y_pred.tolist())
        LBP_Image_score['y_proba_fold'].append(y_pred_prob.tolist())

        LBP_Image_score['y_true_all'].extend(y_test.tolist())
        LBP_Image_score['y_pred_all'].extend(y_pred.tolist())
        LBP_Image_score['y_proba_all'].extend(y_pred_prob.tolist())
        
        
        '''_______________________ Saving Decision for Fusion with Dia Adjustment__________'''
        LBP_Image_score_with_dia['y_true_fold'].append(y_test)
        LBP_Image_score_with_dia['y_pred_fold'].append(y_pred)
        LBP_Image_score_with_dia['y_proba_fold'].append(y_pred_prob)

        LBP_Image_score_with_dia['y_true_all'].extend(y_test)
        LBP_Image_score_with_dia['y_pred_all'].extend(y_pred)
        LBP_Image_score_with_dia['y_proba_all'].extend(y_pred_prob)
    ...
    file_names = ['LBP_Image_score', 'LBP_Image_score_with_dia']
    tag = model_name[:3]
